...
/** Parse Redis value: may be string (JSON) or already object (e.g. some clients auto-parse). */
...
async function addOperator(operator) {
  ...
  const existingIdRaw = await r.get(key('operator:byemail:' + email));
  const existingId = typeof existingIdRaw === 'string' ? existingIdRaw : (existingIdRaw && typeof existingIdRaw === 'object' && existingIdRaw.operatorId ? existingIdRaw.operatorId : String(existingIdRaw));
  if (existingId && existingId !== '[object Object]') {
...
async function createOrGetOperator(email) {
  ...
  const existingIdRaw = await r.get(key('operator:byemail:' + norm));
  const existingId = typeof existingIdRaw === 'string' ? existingIdRaw : (existingIdRaw && typeof existingIdRaw === 'object' && existingIdRaw.operatorId ? existingIdRaw.operatorId : (existingIdRaw ? String(existingIdRaw) : null));
  if (existingId && existingId !== '[object Object]') {
...
async function addAgent(agent) {
  ...
  if (existing) {
    const prev = parseJson(existing);
    if (prev && typeof prev.earnings === 'number') record.earnings = prev.earnings;
  }
...
async function findOperatorByEmail(email) {
  ...
  const operatorIdRaw = await r.get(key('operator:byemail:' + norm));
  const operatorId = typeof operatorIdRaw === 'string' ? operatorIdRaw : (operatorIdRaw && typeof operatorIdRaw === 'object' ? operatorIdRaw.operatorId : (operatorIdRaw ? String(operatorIdRaw) : null));
  if (!operatorId || operatorId === '[object Object]') return null;
...
async function getLeaderboard() {
  ...
  if (!ids || ids.length === 0) {
    ...
    ids = Array.isArray(legacy) ? legacy : (parseJson(legacy) || []);
  }
